...
function ShopbyCatagory() {
  ...
  const data = [
    { img: "images/shop-by-catogory.png" },
    { img: "images/shop-by-catogory-1.png" },
    { img: "images/shop-by-catogory-2.png" },
    { img: "images/shop-by-catogory-3.png" },
  ].map((d, i) => ({
    ...d,
    heading: cats[i % (cats.length || 1)]?.name || "",
    slug:
      cats[i % (cats.length || 1)]?.slug ||
      (cats[i % (cats.length || 1)]?.name || "")
        .toLowerCase()
        .replace(/\s+/g, "-"),
  }));
  return (
    <>
      <div className="max-w-7xl mx-auto p-6 pt-2 text-center">
        <motion.h2
        ...
        >
          {data.map((item, idx) => (
            <Link key={idx} to={`/category/${item.slug}`}>
