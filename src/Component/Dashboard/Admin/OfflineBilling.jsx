...
const OfflineBilling = () => {
  ...
  const subtotalAmount = useMemo(() => {
    ...
    const customPrice =
      selectedProducts.length === 0 ? Number(customItemPrice) || 0 : 0;
    return baseSubtotal + customPrice;
  }, [selectedProducts, customItemPrice]);
  ...
  const handleSubmitBill = async (e) => {
    ...
    if (!customerName.trim()) {
      alert('Please enter the customer name before creating a bill.');
      return;
    }
    if (selectedProducts.length === 0 && !(Number(customItemPrice) > 0)) {
      alert('Please add a product or enter a custom amount before creating a bill.');
      return;
    }

    try {
      const shouldUseCustomItem =
        selectedProducts.length === 0 && Number(customItemPrice) > 0;
      const normalizedProducts = shouldUseCustomItem
        ? [
            {
              id: `custom-${Date.now()}`,
              title: customItemName?.trim() || 'Custom Service',
              name: customItemName?.trim() || 'Custom Service',
              price: Number(customItemPrice) || 0,
              quantity: 1,
            },
          ]
        : selectedProducts;
      const subtotalAmount = normalizedProducts.reduce(
      ...
      await syncOfflineBillToOrders({ ...billData, id: offlineBillRef.id });
      ...
      if (generateInvoiceNow) {
        setSelectedBillForInvoice(billData);
              ...
              <div className="py-4 border-2 border-dashed border-gray-300 rounded-lg px-4">
                ...
                <div className="mt-4 grid gap-4 md:grid-cols-2 text-left">
                  ...
                  <div>
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">
                      This amount will be used when no catalogue product is added.
                    </p>
                  </div>
                </div>
              </div>
